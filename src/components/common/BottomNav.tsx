...
import { Bot, LayoutGrid, PiggyBank, ReceiptText, ShieldCheck, TrendingUp } from 'lucide-react';
...
export const BottomNav: React.FC = () => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 bg-white/95 dark:bg-surface-dark/95 backdrop-blur-2xl border-t border-slate-200/80 dark:border-slate-800 shadow-[0_-4px_24px_-4px_rgba(15,23,42,0.08)] w-full max-w-full overflow-hidden">
      <div className="max-w-md mx-auto px-1 sm:px-2 py-1 sm:py-1.5 grid grid-cols-6 items-center">
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`relative flex flex-col items-center justify-center py-1 sm:py-1.5 px-1 rounded-2xl transition-all ${
                isActive
            ...
            >
              <div className="relative">
                ...
                />
                {item.badge && (
                  <span className="absolute -top-1 -right-2 bg-amber-500 text-white text-[9px] font-black w-4 h-4 rounded-full flex items-center justify-center animate-pulse shadow-xs">
                    {item.badge}
                  </span>
                )}
              </div>
              <span className="text-[9.5px] sm:text-[10px] tracking-tight mt-0.5 truncate text-center leading-tight">
                {item.label}
              </span>
            </button>
          );
